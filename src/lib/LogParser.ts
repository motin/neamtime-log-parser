...
function str_word_count_utf8(string, format = 0) {
    switch (format) {
        ...
        case 2:
            ...
            for (var match of Object.values(matches[0])) {
...
class LogParser {
    ...
    static newline_convert(string, newline) {
        return str_replace([LogParser.NL_WIN, LogParser.NL_MAC, LogParser.NL_NIX], newline, string);
    }

    static textIntoLinesArray(text) //Remove weird skype-produced spaces (hex c2a0 as opposed to hex 20 for ordinary spaces)
    //Normalize line-endings
    {
        text = str_replace("\xA0", " ", text);
        text = this.newline_convert(text, this.NL_NIX);
        var lines = text.split(this.NL_NIX);
        return lines;
    }

    static linesArrayIntoText(lines) {
        return lines.join(this.NL_NIX);
    }

    static readFirstNonEmptyLineOfText(text) {
        var lines = this.textIntoLinesArray(text);
    ...
    supportedTimestampFormats() //the minute-part may be omitted and instead an approx token will be found, which will be replaced before reaching createFromFormat
    //todo: dyn load appr-tokens
    {
        var Ymd_detect_regex = "(1999|2000|2001|2002|2003|2004|2005|2006|2007|2008|2009|2010|2011|2012|2013|2014|2015|2016|2017|2018|2019|2020)-\\d+-\\d+";
        var dmY_detect_regex = "[^\\s\\>>]*-.*-(1999|2000|2001|2002|2003|2004|2005|2006|2007|2008|2009|2010|2011|2012|2013|2014|2015|2016|2017|2018|2019|2020)";
        var His_detect_regex = "(\\d+:\\d+:\\d+)";
        var Hcoloni_detect_regex_accept_approx_token = "(\\d+:([^\\s\\-,:]*|ca|appr))";
        var Hdoti_detect_regex_accept_approx_token = "(\\d+\\.([^\\s\\-,:]*|ca|appr))";
        var Hcoloni_detect_regex = "(\\d+:[^\\s\\-,:]+)";
        var Hdoti_detect_regex = "(\\d+\\.[^\\s\\-,:]+)";
        var iso_timezone_detect_regex = "(Z|\\+\\d\\d:\\d\\d)";
        var utc_timezone_detect_regex = "(\\+UTC)";
        return [{
    ...
    secondsToDuration(seconds, hoursPerDay = 24, daysPerWeek = 7) {
        var vals = {
    ...
    durationToSeconds(duration, hoursPerDay = 24, daysPerWeek = 7) //read and remove weeks
    {
        var total = 0;

        if (strpos(duration, "w") !== false) {
            var p = duration.split("w");
            var weeks = p[0];
    ...
    durationToMinutes(duration, hoursPerDay = 24, daysPerWeek = 7) {
        var seconds = this.durationToSeconds(duration, hoursPerDay, daysPerWeek);
        return seconds / 60;
    }

    durationFromLast(ts, rows_with_timemarkers_handled, rows_with_timemarkers) {
        var previousRowWithTimeMarker = undefined;

        if (rows_with_timemarkers_handled == 0) {
            previousRowWithTimeMarker = undefined;
            var duration_since_last = 0;
        } else {
    ...
    detectTimeStamp(linefordatecheck, metadata) //For debug
    //codecept_debug([__LINE__, compact("metadata")]);
    {
        ...
        for (var supportedTimestampFormat of Object.values(this.supportedTimestampFormats())) //The most straight-forward date format
        //codecept_debug([__LINE__, compact("detect_regex", "linefordatecheck", "m")]);
        {
            var format = supportedTimestampFormat.format;
            var detect_regex = supportedTimestampFormat.detect_regex;
            var accept_approx_token_instead_of_minutes = supportedTimestampFormat.accept_approx_token_instead_of_minutes;
            var detect_regex_date_raw_match_index = supportedTimestampFormat.detect_regex_date_raw_match_index;
            var detect_regex_time_raw_match_index = supportedTimestampFormat.detect_regex_time_raw_match_index;
            preg_match_all(detect_regex, linefordatecheck, m);

            if (!!m && !!m[0]) //var_dump($line, $m);
                {
                    ...
                    if (!is_null(detect_regex_time_raw_match_index)) //If this is a format with only time detection, we use the raw time as the raw date
                        {
                            ...
                            if (accept_approx_token_instead_of_minutes) //In case we entered "approx" instead of minutes, shotgun to the exact hour change:
                                {
                                    if (this.startsWithOptionallySuffixedToken(metadata.time_raw, "approx")) {
    ...
    set_ts_and_date(date_raw, ts, date, linewithoutdate = undefined, datetime) {
        this.lastSetTsAndDateErrorMessage = "";
        date_raw = str_replace(["maj", "okt"], ["may", "oct"], date_raw).trim();

        try {
            var timeZone = this.interpretLastKnownTimeZone();
            ts = this.parseGmtTimestampFromDateSpecifiedInSpecificTimezone(date_raw, timeZone, datetime);
            this.lastUsedTimeZone = timeZone;
        } catch (e) {
            if (e instanceof InvalidDateTimeZoneException) //If invalid timezone is encountered, use UTC and at least detect the timestamp correctly, but make a note about that the wrong timezone was used
                {
        ...
        if (!ts) {
        ...
        } else //new day starts at 06.00 in the morning - schablon
            ...
            //if (date("H:i:s", $ts) == "00:00:00") $midnightoffset = 0; // do not offset when we didn't specify a specific time (yes this takes 00:00-reported times as well - but I can live with that!)
            {
                ...
                var midnightoffset = 6 * 60;
                var interval = DateInterval.createFromDateString(midnightoffset + " minutes");
                var semanticDateTime = clone(datetime);
                semanticDateTime = semanticDateTime.sub(interval);
                date = semanticDateTime.format("Y-m-d");
            }
    }

    interpretLastKnownTimeZone() {
    ...
    parseGmtTimestampFromDateSpecifiedInSpecificTimezone(str, timezone, datetime = undefined) //TODO: Remove expectation of string
    {
        var gmt_timestamp = undefined;
        ...
        for (var supportedTimestampFormat of Object.values(this.supportedTimestampFormats())) {
            var format = supportedTimestampFormat.format;

            if (undefined !== supportedTimestampFormat.pre_datetime_parsing_callback) {
        ...
        if (!datetime) //TODO: Remove expectation of string and this setting of 0 on error
            //var_dump(compact("str","gmt_timestamp"), strtotime($str));
            //die();
            {
                gmt_timestamp = 0;
            } else {
            var gmt_datetime = clone(datetime);
