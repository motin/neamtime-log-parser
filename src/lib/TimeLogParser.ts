...
class TimeLogParser extends LogParser {
    ...
    startsWithOptionallySuffixedToken(haystack, keyword, suffix = "") {
    ...
    //var_dump($date_raw,$m);
    {
        ...
        if (date_raw.length < 8) //var_dump($date_raw);
            {
    ...
    loadContentsFromFile() {
        var tspath = _SERVER.argv[1];
        this.contents = file_get_contents(`${tspath}.txt`);
    }

    isProbableStartStopLine(line, dump = false) {
        var trimmedLine = line.trim();
        var startsWithPauseTokenFollowedByASpace = this.startsWithOptionallySuffixedToken(trimmedLine, "pause", " ");
    ...
    isProbableCommitLogLine(line) {
        var trimmedLine = line.trim();
        return strpos(trimmedLine, "+ ") === 0 || strpos(trimmedLine, "* ") === 0 || strpos(trimmedLine, "^ ") === 0 || strpos(trimmedLine, "! ") === 0;
    }
    ...
    getPreProcessedContents(tz_first, contents) //Phase 0 - skip lines after "#endts"
    //Phase 1 - paus-fixes
    //Phase 2 - missing start-lines
    {
        this.lastKnownTimeZone = tz_first;
        var processed = Array();
        var lines = this.constructor.textIntoLinesArray(contents);
        var nextNeedToBeStart = true;
        var _lines = lines;
        lines = Array();
        ...
        var phase1_source_line_contents_source_line_map = Array();

        for (var source_line_index in lines) //always use trimmed line for comparisons
        //Actual source line is +1
        //Skip empty lines
        {
            var line = lines[source_line_index];
            var trimmedLine = line.trim();
            var source_line = source_line_index + 1;
            var testempty = trimmedLine;

            if (!testempty) {
                processed.push(trimmedLine);
                phase1_source_line_contents_source_line_map[processed.length] = source_line;
                continue;
            }

            var token = this.startsWithOptionallySuffixedToken(trimmedLine, "pause", " ->");
            ...
            if (!!token && strpos(trimmedLine, "->") !== false) //Checking if a timestamp exists before the "->"
                {
                    var rowParts = trimmedLine.split("->");
                    var tokens = this.tokens();
                    var trimmedLinefordatecheck = str_replace(tokens.pause, "", rowParts[0].trim());
                    trimmedLinefordatecheck = str_replace(tokens.approx, "", trimmedLinefordatecheck);

                    if (trimmedLinefordatecheck.length > 1 && strpos(trimmedLinefordatecheck, "min") === false) //var_dump($metadata["date_raw"], $trimmedLinefordatecheck, $ts, $date);
                        //$invalid = empty($date);
                        {
                            ...
                            if (!!metadata.date_raw) //$formatted_date = gmdate("Y-m-d H:i", $ts); //:s
                                {
                                    var implicitMessage = trimmedLinefordatecheck + `, <just before ${token}>`;
        ...
        for (var phase_1_processed_line_index in lines) //always use trimmed line for comparisons
        //Actual source line is +1
        //Get raw source line
        //Skip empty lines
        {
            var line = lines[phase_1_processed_line_index];
            trimmedLine = line.trim();
            var phase_1_processed_line = phase_1_processed_line_index + 1;
            source_line = phase1_source_line_contents_source_line_map[phase_1_processed_line];
            testempty = trimmedLine;

            if (!testempty) {
                processed.push(trimmedLine);
                this.preProcessedContentsSourceLineContentsSourceLineMap[processed.length] = source_line;
                continue;
            }
            ...
            if (nextNeedToBeStart) //We have a certain syntax that can recover a missing start line, let's check for that:
                //@var DateTime $datetime
                {
                    var date_raw = undefined;
                    var ts = undefined;
                    var date = undefined;
                    var trimmedLinewithoutdate = undefined;
                    var notTheFirstRowOfALogComment = undefined;
                    datetime = undefined;
                    this.parseLogComment(trimmedLine, date_raw, ts, date, trimmedLinewithoutdate, notTheFirstRowOfALogComment, datetime);

                    if (!notTheFirstRowOfALogComment) //todo dynamic insertion of apprtokens
                        {
                            var preg = "/^(ca|appr)? ?((\\d)+h)?(\\d+)min/";
                            preg_match(preg, trimmedLinewithoutdate, m);

                            if (!!m) {
                                var apprtoken = m[1];
                                var hours = Math.round(m[3]);
                                var minutes = Math.round(m[4]) + hours * 60;
                            } else //Check for hours without minutes as well
                            ...
                            if (!!minutes) //Here we, instead of start, have a single line with a duration. We can calculate the start from this...
                                ...
                                {
                                    var interval = DateInterval.createFromDateString(minutes + " minutes");

                                    if (!datetime) {
                                        throw new TimeLogParsingException("To be able to calculate the special-syntax-missing-start-line we need to have the DateTime object of the line to subtract from");
                                    }

                                    var probableStartDateTime = datetime.sub(interval);
                                    var probableStart = probableStartDateTime.format("Y-m-d H:i");
    ...
    parseLogComment(line, date_raw, ts, date, linewithoutdate, notTheFirstRowOfALogComment, datetime) //"," is the main separator between date and any written comment...
    {
        this.parseLogCommentWithSeparator(",", line, date_raw, ts, date, linewithoutdate, notTheFirstRowOfALogComment, datetime);
    ...
    parseLogCommentWithSeparator(separator, line, date_raw, ts, date, linewithoutdate, notTheFirstRowOfALogComment, datetime) //Check if we have a valid date already
    ...
    //since some timestamp formats may include the seperator (at most once)
    {
        ...
        datetime = undefined;
        this.parseLogCommentDateRawCandidate(date_raw, ts, date, linewithoutdate, notTheFirstRowOfALogComment, datetime);
    ...
    parseLogCommentDateRawCandidate(date_raw, ts, date, linewithoutdate, notTheFirstRowOfALogComment, datetime) //invalidate pure numbers (including those with fractional parts) if there is no comment on the other side - probably not a real log comment
    //invalidate lines without a valid date
    {
        ...
        if (!!m[0]) {
            var firstMatch = m[0];
            var trimmedDateRaw = date_raw.trim();

            if (str_replace([".", ","], "", firstMatch) === str_replace([".", ","], "", trimmedDateRaw) && !linewithoutdate.trim()) //Due to some odd logic in some other file, we also can't set ts and date for this row
                {
    ...
    addTimeMarkers() {
        ...
        var rows_with_timemarkers = Array();
        ...
        this.parsePreProcessedContents(this.preProcessedContents, rows_with_timemarkers, not_parsed);
        this.contentsWithTimeMarkers = this.generateStructuredTimeMarkedOutputBasedOnParsedRowsWithTimeMarkers(rows_with_timemarkers, not_parsed);
    ...
    parsePreProcessedContents(preProcessedContents, rows_with_timemarkers, not_parsed) {
        var original_unsorted_rows = Array();
        var rows_with_timemarkers_handled = 0;
        var lines = this.constructor.textIntoLinesArray(preProcessedContents);

        for (var preprocessed_contents_source_line_index in lines) //Actual source line is +1
        //skip empty rows
        ...
        {
            var line = lines[preprocessed_contents_source_line_index];
            var trimmedLine = line.trim();
            var preprocessed_contents_source_line = preprocessed_contents_source_line_index + 1;

            if (trimmedLine == "") {
                continue;
            }

            var source_line = this.preProcessedContentsSourceLineContentsSourceLineMap[preprocessed_contents_source_line];
            ...
            var line_with_comment = trimmedLine;
            ...
            var notTheFirstRowOfALogComment = undefined;
            var datetime = undefined;
            this.parseLogComment(line, date_raw, ts, date, linewithoutdate, notTheFirstRowOfALogComment, datetime);
            var timezone = new DateTimeZone("UTC");
            datetime = new DateTime();
            datetime.setTimezone(timezone);
            datetime.setTimestamp(ts);
            var formatted_date = datetime.format("Y-m-d H:i");
            var log = Array();
            var lastKnownTimeZone = this.lastKnownTimeZone;
            var lastUsedTimeZone = this.lastUsedTimeZone;
            var lastSetTsAndDateErrorMessage = this.lastSetTsAndDateErrorMessage;
            var metadata = compact("rows_with_timemarkers_handled", "line", "line_with_comment", "formatted_date", "date", "date_raw", "ts", "log", "source_line", "preprocessed_contents_source_line_index", "lastKnownTimeZone", "lastUsedTimeZone", "lastSetTsAndDateErrorMessage");

            if (this.interpretLastKnownTimeZone() !== this.lastUsedTimeZone) {
                var methodName = "parsePreProcessedContents";
                metadata.log.push(`Invalid timezone ('${this.lastKnownTimeZone}') encountered when parsing a row (source line: ${source_line}). Not treating this row as valid time-marked row`);
                metadata.log.push("Sent to not_parsed in " + methodName);
                not_parsed.push(metadata);
            }

            var isNewRowWithTimeMarker = false;
            var notTheFirstRowOfALogCommentAndProbableStartStopLine = notTheFirstRowOfALogComment && this.isProbableStartStopLine(line);
            var isTheFirstRowWithTimeMarker = !rows_with_timemarkers[rows_with_timemarkers_handled - 1];
            var hasAPreviousRowWithTimemarker = !isTheFirstRowWithTimeMarker;
            var previousRowWithTimemarkerHasTheSameDate = hasAPreviousRowWithTimemarker && rows_with_timemarkers[rows_with_timemarkers_handled - 1].formatted_date == formatted_date;

            if (notTheFirstRowOfALogCommentAndProbableStartStopLine) {
                isNewRowWithTimeMarker = true;
                this.processNotTheFirstRowOfALogCommentAndProbableStartStopLine(line, metadata, rows_with_timemarkers, rows_with_timemarkers_handled, not_parsed, isNewRowWithTimeMarker);
            } else if (notTheFirstRowOfALogComment) {
                this.processAdditionalLogCommentRowUntilNextLogComment(line, rows_with_timemarkers, rows_with_timemarkers_handled);
                isNewRowWithTimeMarker = false;
            } else if (previousRowWithTimemarkerHasTheSameDate) {
                this.processAdditionalLogCommentRowUntilNextLogComment(line, rows_with_timemarkers, rows_with_timemarkers_handled);
                isNewRowWithTimeMarker = false;
            } else {
                var theFirstRowOfALogComment = true;
                this.processTheFirstRowOfALogComment(ts, isNewRowWithTimeMarker, metadata, rows_with_timemarkers, rows_with_timemarkers_handled, not_parsed);
            }
    ...
    processNotTheFirstRowOfALogCommentAndProbableStartStopLine(line, metadata, rows_with_timemarkers, rows_with_timemarkers_handled, not_parsed, isNewRowWithTimeMarker) //Assume true
    //Check if it's a pause with written duration
    {
        var startsWithPauseToken = this.startsWithOptionallySuffixedToken(line, "pause");
        var isTheFirstRowWithTimeMarker = !rows_with_timemarkers[rows_with_timemarkers_handled - 1];
        var probableStartStopLineIsIndeedStartStopLineWithSaneTimestamp = true;
        var pauseWithWrittenDuration = startsWithPauseToken && strpos(line, "min") !== false;

        if (pauseWithWrittenDuration) {
            this.processNotTheFirstRowOfALogCommentAndProbableStartStopLine_pauseWithWrittenDuration(metadata, rows_with_timemarkers, rows_with_timemarkers_handled, not_parsed, probableStartStopLineIsIndeedStartStopLineWithSaneTimestamp);
        } else {
            this.processNotTheFirstRowOfALogCommentAndProbableStartStopLine_notPauseWithWrittenDuration(line, startsWithPauseToken, metadata, rows_with_timemarkers, rows_with_timemarkers_handled, not_parsed, probableStartStopLineIsIndeedStartStopLineWithSaneTimestamp, isNewRowWithTimeMarker);
        }

        if (!probableStartStopLineIsIndeedStartStopLineWithSaneTimestamp) //If last valid row is not enabled - throw a large exception, is this log contents?
            {
            ...
            } else //We keep track of sessions starts for double-checking that time has registered on each of those dates
            {
                var startsWithStartTokenFollowedByASpace = this.startsWithOptionallySuffixedToken(line, "start", " ");
    ...
    processNotTheFirstRowOfALogCommentAndProbableStartStopLine_pauseWithWrittenDuration(metadata, rows_with_timemarkers, rows_with_timemarkers_handled, not_parsed, probableStartStopLineIsIndeedStartStopLineWithSaneTimestamp) //preg_match('/([^-]-[^-]-2009) ([^:]*):([^c ]*)/', $linefordatecheck, $m);
    //$metadata["duration_search_preg_debug"] = compact("linefordurationcheck","m");
    {
        var methodName = "processNotTheFirstRowOfALogCommentAndProbableStartStopLine_pauseWithWrittenDuration";
        ...
        metadata.highlight_with_newlines = true;
        var parts = metadata.line.split("->");
        var linefordurationcheck = parts[0];
        preg_match_all("/(([0-9])*h)?([0-9]*)min/", linefordurationcheck, m);

        if (!!m && !!m[0]) //var_dump($line, $m, $rows_with_timemarkers_handled, $rows_with_timemarkers);
            {
                metadata.log.push("found pause duration, adding to accumulated pause duration (if any)");

                if (!!rows_with_timemarkers[rows_with_timemarkers_handled - 1].pause_duration) {
                    metadata.pause_duration = Math.round(rows_with_timemarkers[rows_with_timemarkers_handled - 1].pause_duration);
                } else {
                    metadata.pause_duration = 0;
                }

                metadata.pause_duration += 60 * (Math.round(m[2][0]) * 60 + Math.round(m[3][0]));
                metadata.ts_is_faked = false;
                probableStartStopLineIsIndeedStartStopLineWithSaneTimestamp = true;
            } else //To easily see patterns amongst these lines
            {
    ...
    processNotTheFirstRowOfALogCommentAndProbableStartStopLine_notPauseWithWrittenDuration(line, startsWithPauseToken, metadata, rows_with_timemarkers, rows_with_timemarkers_handled, not_parsed, probableStartStopLineIsIndeedStartStopLineWithSaneTimestamp, isNewRowWithTimeMarker) //Try to find a valid timestamp
    ...
    {
        var methodName = "processNotTheFirstRowOfALogCommentAndProbableStartStopLine_notPauseWithWrittenDuration";
        var linefordatecheck = line;
        ...
        var date = undefined;
        this.set_ts_and_date(metadata.date_raw, ts, date, undefined, datetime);
        var validTimestampFound = !!date;
        var thisTimestampIsLaterOrSameAsPreviousRowWithTimeMarker = undefined;
        var duration_since_last = undefined;

        if (validTimestampFound === true) //Get duration from last count
            {
                duration_since_last = this.durationFromLast(ts, rows_with_timemarkers_handled, rows_with_timemarkers);
        ...
        if (validTimestampFound !== true) //Treat as AdditionalLogCommentRowUntilNextLogComment if not a pause line
            {
                metadata.log.push("Did NOT find a valid timestamp in a probable start/pause-row. Not treating this row as a time-marked row");
                metadata.log.push(`Line: ${line}`);

                if (!startsWithPauseToken) {
                    metadata.log.push("Sent to processAdditionalLogCommentRowUntilNextLogComment in " + methodName);
                    this.processAdditionalLogCommentRowUntilNextLogComment(line, rows_with_timemarkers, rows_with_timemarkers_handled);
                    isNewRowWithTimeMarker = false;
                } else //To easily see patterns amongst these lines
                    {
        ...
        if (thisTimestampIsLaterOrSameAsPreviousRowWithTimeMarker === false) //To easily see patterns amongst these lines
            {
                metadata.log.push("Timestamp found in probable start/pause-row, but was earlier than last found");
                metadata.log.push(`Line: ${line}`);
                var last = clone(datetime);
                last.add(new DateInterval("PT" + Math.abs(duration_since_last) + "S"));
                metadata.log.push(`Timestamp found: ${datetime.format("Y-m-d H:i:s")} vs last found (based on duration since last which is ${duration_since_last}): ${last.format("Y-m-d H:i:s")}`);
                metadata.log.push("Sent to not_parsed in " + methodName);
                not_parsed.push(metadata);
            }

        if (validTimestampFound && thisTimestampIsLaterOrSameAsPreviousRowWithTimeMarker) {
            metadata.log.push("Found a valid timestamp in probable start/pause-row... interpreted as start/pause-row...");
            metadata.ts = ts;
            metadata.date = date;
            var timezone = new DateTimeZone("UTC");
            datetime = new DateTime();
    ...
    processAdditionalLogCommentRowUntilNextLogComment(line, rows_with_timemarkers, rows_with_timemarkers_handled) {
        if (!(undefined !== rows_with_timemarkers[rows_with_timemarkers_handled - 1])) {
    ...
    processTheFirstRowOfALogComment(ts, isNewRowWithTimeMarker, metadata, rows_with_timemarkers, rows_with_timemarkers_handled, not_parsed) //Get duration from last count
    {
        var duration_since_last = this.durationFromLast(ts, rows_with_timemarkers_handled, rows_with_timemarkers);

        if (duration_since_last < 0) //Debug log info
            {
                ...
                last.add(new DateInterval("PT" + Math.abs(duration_since_last) + "S"));
                metadata.log.push(`Timestamp found: ${datetime.format("Y-m-d H:i:s")} vs last found (based on duration since last which is ${duration_since_last}): ${last.format("Y-m-d H:i:s")}`);
                var previousRowWithTimeMarker = rows_with_timemarkers[rows_with_timemarkers_handled - 1];
                metadata.log.push(`$previousRowWithTimeMarker line: ${previousRowWithTimeMarker.line}`);
                metadata.log.push("sent to not_parsed in processTheFirstRowOfALogComment");
                not_parsed.push(metadata);
            } else if (duration_since_last > 24 * 60 * 60) //Warn on unlikely large entries (> 24h) - likely typos
            //TODO: Make limit configurable
            {
                metadata.log.push("excessive duration since last: " + this.secondsToDuration(duration_since_last));
                not_parsed.push(metadata);
                isNewRowWithTimeMarker = true;
            } else {
    ...
    generateStructuredTimeMarkedOutputBasedOnParsedRowsWithTimeMarkers(rows_with_timemarkers, not_parsed) //Generate structured log output
    {
        if (!rows_with_timemarkers) {
            throw new TimeLogParsingException("No rows parsed...");
        }

        var last = rows_with_timemarkers.pop();

        if (false) //The last pause was started some time after the last log message
            {} else {
    ...
    generateStructuredTimeMarkedOutput(rows_with_timemarkers, not_parsed) //Remove "paus->" from not_parsed array
    {
        var contentsWithTimeMarkers = "";
        contentsWithTimeMarkers += ".:: Uncategorized\n";

        for (var k in rows_with_timemarkers) {
            var metadata = rows_with_timemarkers[k];

            if (undefined !== metadata.highlight_with_newlines && metadata.highlight_with_newlines) {
                contentsWithTimeMarkers += this.constructor.NL_NIX;
            }

            contentsWithTimeMarkers += "\t";

            if (undefined !== metadata.duration_since_last && !is_null(metadata.duration_since_last)) //Remove any known pause durations
                {
                    ...
                    if (metadata.duration_since_last < 0) {
                        metadata.log.push("negative duration since last");
                        metadata.log.push("sent to not_parsed in generateStructuredTimeMarkedOutput");
                        not_parsed.push(metadata);
                    }
                    ...
                    if (!!rows_with_timemarkers[k - 1] && !!rows_with_timemarkers[k - 1].ts_is_faked) //Treat this situation as invalid
                        {
                            contentsWithTimeMarkers += " {!} ";
                            var previousRow = rows_with_timemarkers[k - 1];
                            metadata.log.push("duration since last is based on fake/interpolated timestamp");
        ...
        if (!!not_parsed) {
            for (var k in not_parsed) {
                var metadata = not_parsed[k];
                var token = this.startsWithOptionallySuffixedToken(metadata.line + "|$", "pause", "->|$");
    ...
    notParsedAddTimeMarkersErrorSummary(not_parsed) {
        if (!not_parsed) {
        ...
        for (var v of Object.values(not_parsed)) {
            if (Array.isArray(v) && !!v.source_line) {
                summary[v.source_line] = v;
            } else {
                throw new TimeLogParsingException("The unparsed contents did not contain information about the source line: " + print_r(v, true));
            }
        }
    ...
    notParsedTimeReportErrorSummary(not_parsed) {
        if (!not_parsed) {
            throw new TimeLogParsingException("Can not summarize not-parsed errors without any unparsed contents");
        }
    ...
    detectCategories() {
        this.categories = Array();
        var lines = this.constructor.textIntoLinesArray(this.contentsWithTimeMarkers);

        for (var line of Object.values(lines)) //skip empty rows
        {
            var trimmedLine = line.trim();
            ...
            if (strpos(line, ".::") === 0) {
                var catneedle = str_replace(".::", "", trimmedLine).trim();
                this.categories.push(catneedle);
            }
        }
    }

    generateTimeReport() //Fill out and sort the times-array
    ...
    //var_dump($times);
    {
        ...
        for (var lineno in lines) //skip empty rows
        ...
        //Save a useful form of the time-marked rows that build up the hours-sum:
        {
            ...
            var date_raw = parts.shift();
            var linewithoutdate = parts.join(",");
            var datetime = undefined;
            var _ = this.lastKnownTimeZone;
            this.lastKnownTimeZone = "UTC";
            this.set_ts_and_date(date_raw, ts, date, linewithoutdate, datetime);
            this.lastKnownTimeZone = _;
            parts = line.split(" | ");
            var beforeVertLine = parts[0];

            if (this.isProbableStartStopLine(beforeVertLine)) {
                continue;
            }

            var invalid = !date || strpos(linewithoutdate, "min") === false || ts < time() - 24 * 3600 * 365 * 10;
            ...
            var tokens = this.tokens();
            var duration = str_replace(tokens.approx, "", parts[0]).trim() + "min";
            var time = this.durationToMinutes(duration);
            invalid = !time && time !== 0;
            ...
            var hours_rounded = Math.round(time / 60, 2);
            var hours = time / 60;
            var first = parts.shift();
            var text = parts.join("min");
            invalid = !text;

            if (invalid) //var_dump($first, $parts, $line); die();
            ...
            var sourceComment = compact("category", "timemarker", "linewithoutdate", "ts", "date_raw", "date", "hours_rounded", "hours", "text");
        ...
        contentsOfTimeReport += "Date;" + this.categories.join(" (rounded);") + " (rounded);" + this.categories.join(";") + ";Log_Items\n";

        for (var date in times) //Gotta limit the amount of data
        //
        ...
        {
            var hours = times[date];
            var activities = Array.from(hours.text).join(" | ");
            ...
            var hours_by_category_rounded = "";

            for (var c of Object.values(this.categories)) {
                var hours_exact = undefined !== hours[c] ? hours[c] : 0;
                hours_rounded = Math.round(hours_exact, 2);
                hours_by_category_rounded += hours_rounded + ";";
            }

            var hours_by_category = "";
            ...
            contentsOfTimeReport += date + ";" + hours_by_category_rounded + hours_by_category + activities + this.constructor.NL_NIX;
        }

        this.notParsedTimeReport = not_parsed;

        for (var line of Object.values(not_parsed)) //maybe attempt to add some debugging metadata here?
        {}

        this.contentsOfTimeReport = contentsOfTimeReport;
    }

    addZeroFilledDates(times) //Find time span:
    //codecept_debug(compact("period", "firstdatefound", "lastdatefound", "interval"));
    {
        var firstdatefound = undefined;
        var lastdatefound = undefined;
        var times_ = times;
        times = Array();
        var timezone = new DateTimeZone("UTC");

        for (var date in times_) {
            var hours = times_[date];
            var datetime = DateTime.createFromFormat("Y-m-d H:i:s", date + " 00:00:00", timezone);
        ...
        var period = new DatePeriod(firstdatefound, interval, lastdatefound);
        times_ = times;
        times = Array();

        for (var dt of Object.values(period)) {
            var xday = dt.format("Y-m-d");
    ...
    //} while (!$last["ts_is_faked"]);
    {
        ...
        var rows_with_timemarkers = this.debugAddTimeMarkers.rows_with_timemarkers;
        var start = rows_with_timemarkers.shift();
        var startts = start.ts;
        var name = start.date_raw;
        var last = rows_with_timemarkers.pop();
        var lastts = last.ts;
        var leadtime = lastts - startts;
        var times = this.debugGenerateTimeReport.times;
        var hours_total = 0;

        for (var time of Object.values(times)) {
            for (var category of Object.values(this.categories)) {
                hours_total += time[category];
            }
        }

        var hours_leadtime = Math.round(leadtime / 60 / 60, 2);
        var nonhours = Math.round(hours_leadtime - hours_total, 2);
    ...
    //generate and get output in string, for testing?
    {
        ...
        {
            let _tmp_0 = this.sessions;

            for (var k in _tmp_0) //Only sessions with work hours
            //var_dump($session["metadata"]);
            //Inactivating until have a solution for timezone split issues
            {
                var session = _tmp_0[k];
                ...
                var vevents = Array();
                var start = gmdate("H:i", session.metadata.startts);
                var last = gmdate("H:i", session.metadata.lastts);
                var startdate = gmdate("Y-m-d", session.metadata.startts);
                var lastdate = gmdate("Y-m-d", session.metadata.lastts);
                var startts = session.metadata.startts;
                var lastts = session.metadata.lastts;

                if (false && startdate != lastdate) {
                    var days = (strtotime(lastdate) - strtotime(startdate)) / (24 * 3600);
                    start = gmdate("Y-m-d H:i", session.metadata.startts);
                    last = gmdate("Y-m-d H:i", session.metadata.lastts);

                    for (var i = 0; i <= days; i++) {
                        var idate = gmdate("Y-m-d", startts + 24 * 3600 * i);
                ...
                var summary = session.metadata.hours_total + " project hours (" + start + "->" + last;
                ...
                summary += ") ";
                var description = "Start datetime: " + start + this.constructor.NL_NIX;
                description += "End datetime: " + last + this.constructor.NL_NIX;
                description += "Work hours: " + session.metadata.hours_total + this.constructor.NL_NIX;
                description += "Non-work hours: " + session.metadata.nonhours + this.constructor.NL_NIX;
                ...
                for (var vevent of Object.values(vevents)) //$e->setProperty( 'categories'
                //, 'WORK' );                   // catagorize
        ...
        var str = v.createCalendar();
        str = utf8_encode(str);
        return str;
    }

    setVcalendarDt(e, field, ts) {
        e.setProperty(field, gmdate("Y", ts), gmdate("m", ts), gmdate("d", ts), gmdate("H", ts), gmdate("i", ts), gmdate("s", ts));
    }

    someMeaningInItAll(str) //array_count_values() returns an array using the values of the input array as keys and their frequency in input as values.
    {
        if (!str) {
            return "";
        }
        ...
        for (var key in word_count) {
            var val = word_count[key];
            var pausetoken = this.startsWithOptionallySuffixedToken(key, "pause");
