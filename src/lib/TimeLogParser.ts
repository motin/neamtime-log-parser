...
class TimeLogParser extends LogParser {
  ...
  public getPreProcessedContents(
    tz_first,
    contents // Phase 0 - skip lines after "#endts" // Phase 1 - paus-fixes
  ...
  public parseLogCommentWithSeparator(
    ...
    linewithoutdate,
    notTheFirstRowOfALogComment,
    datetime // Check if we have a valid date already // If not, allow one more separated chunk into the date_raw and try again
  ...
  public processNotTheFirstRowOfALogCommentAndProbableStartStopLine_notPauseWithWrittenDuration(
    ...
    not_parsed,
    probableStartStopLineIsIndeedStartStopLineWithSaneTimestamp,
    isNewRowWithTimeMarker // Try to find a valid timestamp // Remove the pause specification before attempting to find a timestamp
  ) // @var DateTime $datetime
